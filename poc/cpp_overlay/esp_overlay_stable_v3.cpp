...
constexpr double kAimDeadzonePixels = 4.0;
constexpr double kAimFineControlWindowPixels = 24.0;
constexpr double kAimFineGain = 0.34;
constexpr double kAimCoarseGain = 0.56;
...
constexpr double kAimMinInputIntervalSeconds = 0.0335;
constexpr double kAimMaxRelativeVelocity = 14.0;
constexpr double kAimSnapshotLeadSeconds = 0.0;
constexpr double kAimMaxSnapshotLeadSeconds = 0.030;
constexpr double kAimMaxLeadMeters = 0.35;
constexpr double kAimMaxLeadScreenPixels = 24.0;
...
constexpr double kAimLatencyCompensationSeconds = 0.0;
constexpr double kAimMaxLatencyCompensationSeconds = 0.020;
constexpr double kAimMaxLatencyCompensationMeters = 0.25;
...
constexpr LONG kAimMaxHipMouseStep = 420;
constexpr LONG kAimMaxScopedMouseStep = 220;
constexpr double kAimVerticalGain = 0.32;
constexpr double kAimScopedVerticalGain = 0.20;
constexpr double kAimVerticalDeadzonePixels = 4.0;
constexpr LONG kAimMaxHipVerticalStep = 220;
constexpr LONG kAimMaxScopedVerticalStep = 120;
...
constexpr bool kAimUseSyntheticCalibration = false;
...
void CompleteAimCalibrationProbe(const Snapshot& current) {
    if (!kAimUseSyntheticCalibration) {
...
bool BuildAimCandidate(const Snapshot& snapshot, const Target& target, const RECT& client,
    ...
    const double aimDistance = std::hypot(aimX - width * 0.5, aimY - height * 0.5);
    if (!std::isfinite(aimDistance) || aimDistance > maximumDistance) {
        return false;
    }
    *output = {&target, aimX, aimY, aimWorld, aimDistance};
    return true;
}
...
bool BuildCalibratedAimMouseDelta(const Snapshot& snapshot, const RECT& client,
    ...
    const int minimumLearnedSamples = IsScopedFov(snapshot.fov) ? 16 : 16;
    ...
    double pixelX = static_cast<double>(aimPoint.x) - clientWidth * 0.5;
    double pixelY = static_cast<double>(aimPoint.y) - clientHeight * 0.5;
    if (std::abs(pixelX) <= kAimDeadzonePixels) {
        pixelX = 0.0;
    }
    ...
    const double verticalGain = IsScopedFov(snapshot.fov)
        ...
        : kAimVerticalGain;
    const double downwardBoost = 1.0;
...
void ApplyAimAssist(const Snapshot& snapshot, const RECT& client) {
    ...
    const double aimRadius = AimFovRadius(client);
    if (!std::isfinite(remainingDistance) || remainingDistance > aimRadius) {
        ClearAimLock();
