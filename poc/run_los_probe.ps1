...
& $python (Join-Path $root 'remote_py_run.py') `
    --pid $gamePid `
    --code-file (Join-Path $root 'ctf_los_probe_code.py') `
    --out (Join-Path $root 'remote-py-run-los-probe-current.json') `
    --timeout-ms 20000 `
    *> (Join-Path $root 'remote-py-run-los-probe-current.stdout.txt')
