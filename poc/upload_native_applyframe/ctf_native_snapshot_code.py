...
def _ctf_native_max_target_distance(state):
    ...
    _ctf_native_native_aim = bool(state.get("native_aim_enabled", True))
...
def _ctf_native_write_snapshot(state):
    ...
    _ctf_native_frame = _ctf_native_apply_native_aim(
        state,
        _ctf_native_camera,
        _ctf_native_frame,
        _ctf_native_camera_pos,
        _ctf_native_screen,
        _ctf_native_rows,
    )
...
def _ctf_native_tick(*_ctf_native_args, **_ctf_native_kwargs):
    ...
    try:
        ...
        if _ctf_native_state["tick"] == 1 or _ctf_native_state["tick"] % 240 == 0:
            _ctf_native_log(
                "snapshot tick={} targets={} players={} bots={} culled={} range={:.0f} native_aim={} applied={} lock={} apply_frame_ok={} source_players={} source_robots={} weapon={} velocity={:.3f}".format(
...
def _ctf_native_install():
    ...
    _ctf_native_write_snapshot(_ctf_native_state)
    _ctf_native_log(
        "INSTALL_OK hz=60 timer_id={!r} apply_frame_ok={} apply_frame_error={!r}".format(
